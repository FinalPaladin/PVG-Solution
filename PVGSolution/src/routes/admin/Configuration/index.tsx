...
export default function ConfigurationPage(): JSX.Element {
    ...
    const loadForm = async () => {
        ...
        let objConfig = Object.fromEntries(res?.result?.data.map(item => [item.key, item.value]) ?? []) as unknown as IObjConfigurationModel;
    ...
    const handleSave = async () => {
        try
        {
            let data = Object.entries(config).map(([key, value]) => ({
                key,
                value
            }));
            ...
            let res = await configsSave(formData);
    ...
    const AddImg = (_key: string, _img: File) => {
        ...
        let imgItem = [...dataImg].find(x => x.key === _key);
        if(imgItem)
        {
